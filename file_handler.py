...
def write_to_csv_file(file_object, text, alias):
    ...
    :param alias: can be empty string. useful for info such as coordinates to give an alias (location name) to them.
    :param text: the given text to write to the file.
    :param file_object: the file object, created when opening a file (e.g., "with open(file) as file_object:")
    :return: Nothing
    """
    try:
...
class FileHandler:
    ...
    def open_label_file(self, label, text, alias):
        ...
        :param label: This will be the name for the label file. This is the label associated with a word
